...
const VenueManager = () => {
  ...
  const handleCategoryChange = (index, field, value) => {
    ...
    updatedCategories[index][field] = field === 'rowCount' ? parseInt(value) : value;
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Aisle After Column</label>
                <input
                  type="number"
                  name="aisleAfterCol"
                  min="1"
                  ...
                  <div className="grid grid-cols-4 gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleCategoryChange(index, 'rowCount', parseInt(e.target.value))}
                        className="w-full px-2 py-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-sm"
                        required
                        disabled={isEditing}
                      />
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="flex flex-wrap gap-4 mb-4">
            {formData.categories.map((cat, i) => (
              <div key={i} className="flex items-center">
                ...
                ></span>
                <span className="text-sm">{cat.name} ({cat.rowCount} rows)</span>
              </div>
            ))}
            <div className="flex items-center">
              <span 
                className="w-4 h-4 inline-block mr-1 bg-red-500 border border-gray-300"
              ></span>
          ...
          {selectedSeats.length > 0 && (
            <div className="mt-4 p-3 bg-yellow-50 rounded-md">
              <h4 className="text-sm font-medium mb-1">Marked as Unavailable:</h4>
              <p className="text-xs text-gray-700 break-all">{selectedSeats.join(', ')}</p>
            </div>
          )}
        </div>
      </div>
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {venue.layoutType} ({venue.seatMap.rows}x{venue.seatMap.cols})
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    Categories: {venue.seatMap.categories.map(c => c.name).join(', ')}
                  </p>
                </div>
                <button
