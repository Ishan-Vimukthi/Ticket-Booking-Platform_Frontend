...
const VenueManager = () => {
  ...
  const fetchVenues = async () => {
    setIsLoading(true);
    try {
      const res = await axios.get(`${apiBaseUrl}/api/venues`);
  ...
  const generatePreview = async () => {
    try {
      const res = await axios.post(`${apiBaseUrl}/api/venues/preview`, {
        rows: formData.rows,
        cols: formData.cols,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        const res = await axios.put(`${apiBaseUrl}/api/venues/${currentVenueId}`, {
          unavailableSeats: formData.unavailableSeats,
        });
        setVenues(venues.map((v) => (v._id === currentVenueId ? res.data.data : v)));
        alert("Venue updated successfully!");
      } else {
        const res = await axios.post(`${apiBaseUrl}/api/venues`, formData);
        setVenues(prevVenues => Array.isArray(prevVenues) ? [...prevVenues, res.data.data] : [res.data.data]);
  ...
  const deleteVenue = async (id) => {
    if (window.confirm("Are you sure you want to delete this venue?")) {
      try {
        await axios.delete(`${apiBaseUrl}/api/venues/${id}`);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mt-12">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-medium">Existing Venues</h3>
          <button
